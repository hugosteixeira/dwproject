...
class HastagMining:
    ...
    FINISH_TIMESTAMP = math.ceil(time.time())
    STEPP_TIMESTAMP = 86400 #o range para captura da lista de tweets ex: um dia, uma hora (cada loop roda uma dia... uma hora...)
    ...
    def mining(self, stamp_start, stamp_finish):
        ...
        while stamp_start < stamp_finish :
            ...
            for tweet in reversed(elementList):
                ...
                dao = Dao()
                dao.insert('manager', ['hastag', 'idTweet', 'candidato', 'timeStamp'],[self._hastag, idTweet, self._candidato, tweetTimeStamp])
            ...
            stamp_start = int(stamp_start) + self.STEPP_TIMESTAMP + 1
